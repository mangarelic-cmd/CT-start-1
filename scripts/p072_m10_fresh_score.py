...
OUT=Path(f'p072-primary-{SHARD}'); OUT.mkdir(exist_ok=True)
EXCLUDED=set(json.loads(Path('scripts/p072_excluded.json').read_text())['excluded_prior_unique'])
...
SEED='P072_M10_LOCAL_GAS_PRESSURE'
...
selection={'schema':'P072_FROZEN_METADATA_ONLY_SELECTION_V1','seed':SEED,'metadata_source_commit':'1a200f4588addaad2ae3d53d438d9e1946c09294','required_columns':REQ,'optional_carrier':'gas_injection.pressure','optional_carrier_used_for_selection':False,'excluded_count':len(EXCLUDED),'eligible_count':len(elig),'primary':PRIMARY,'reserves':RESERVE,'fresh_fair_mast_values_read_before_freeze':False,'fresh_Te_values_read_before_freeze':False,'model':'M10_LOCAL_GAS_PRESSURE_GATE_FROZEN_P071','comparators':['M9','M8','M7'],'missing_pressure':'exact fallback M10=M9; UNKNOWN is not zero','refit':False,'retune':False}
(OUT/'FROZEN_SELECTION_P072.json').write_text(json.dumps(selection,indent=2,sort_keys=True)+'\n')
...
P_MU=2.507404867899074e18; P_SD=2.753072212713895e18
M10_D=np.array([-0.19264236293694317,0.24398925923429002],float)
...
def score_shot(sid):
    rec={'shot_id':sid,'split':'P072_FRESH','refit':False,'retune':False,'models':['M7','M8','M9','M10_LOCAL_GAS_PRESSURE'],'pressure_required_for_selection':False,'pressure_missing_behavior':'M10_EQUALS_M9','unknown_is_zero':False}
    try:
        ...
        gi=xr.open_zarr(mp,group='gas_injection',consolidated=None); ti,vi=series(gi,'inboard_total'); to,vo=series(gi,'outboard_total'); tp,vp=series(gi,'pressure')
        eq=xr.open_zarr(mp,group='equilibrium',consolidated=None); tb,vb=series(eq,'beta_pol')
        if any(x is None for x in [tg,tn,tr,ti,to,tb]): raise ValueError('REQUIRED_STATE_SERIES_FAIL')
        ...
        pressure=interp(tp,vp,td) if tp is not None else np.full(len(td),np.nan)
        dgin=deriv_on_finite(td,gin); dgout=deriv_on_finite(td,gout)
        Y=[];P7=[];P8=[];P9=[];P10=[];TT=[];PRES=[];PA=[];G9=[];G10=[]
        for j,t in enumerate(td):
            ...
            hasp=bool(np.isfinite(pressure[j]))
            if hasp:
                zp=(pressure[j]-P_MU)/P_SD; g10=float(np.clip(g9+M10_D[0]+M10_D[1]*zp,0,1))
            else:
                g10=g9
            m10=m7+g10*c8
            Y.append(y);P7.append(m7);P8.append(m8);P9.append(m9);P10.append(m10);TT.append(np.full(m.sum(),t));PRES.append(np.full(m.sum(),pressure[j] if hasp else np.nan));PA.append(np.full(m.sum(),hasp));G9.append(np.full(m.sum(),g9));G10.append(np.full(m.sum(),g10))
        if not Y: raise ValueError('NO_PAIRABLE_M9_SUPPORT')
        y=np.concatenate(Y); p7=np.concatenate(P7); p8=np.concatenate(P8); p9=np.concatenate(P9); p10=np.concatenate(P10); tt=np.concatenate(TT); pa=np.concatenate(PA).astype(bool)
        e7=float(np.mean((y-p7)**2)); e8=float(np.mean((y-p8)**2)); e9=float(np.mean((y-p9)**2)); e10=float(np.mean((y-p10)**2))
        np.savez_compressed(OUT/f'SHOT_{sid}_P072_M10_RAW_SCORE_PACKET.npz',y_log_te=y,m7_pred=p7,m8_pred=p8,m9_pred=p9,m10_pred=p10,time_s=tt,pressure=np.concatenate(PRES),pressure_available=pa,g9=np.concatenate(G9),g10=np.concatenate(G10))
        rec.update({'status':'USABLE','n_pairs':int(len(y)),'pressure_active_pairs':int(pa.sum()),'pressure_active_fraction':float(pa.mean()),'pressure_active_any':bool(pa.any()),'m7_mse':e7,'m8_mse':e8,'m9_mse':e9,'m10_mse':e10,'delta_m10_minus_m9':e10-e9,'delta_m10_minus_m7':e10-e7,'m10_beats_m9':bool(e10<e9),'m10_beats_m7':bool(e10<e7)})
...
(OUT/f'RECEIPTS_P072_{SHARD}.json').write_text(json.dumps(receipts,indent=2,sort_keys=True)+'\n')
(OUT/f'SUMMARY_P072_{SHARD}.json').write_text(json.dumps({'schema':'P072_PRIMARY_SHARD_SUMMARY_V1','shard':SHARD,'primary':prim,'reserve_owned':res,'reserve_activated':activated,'usable':sum(r['status']=='USABLE' for r in receipts),'failed':sum(r['status']!='USABLE' for r in receipts),'pressure_active_shots':sum(bool(r.get('pressure_active_any')) for r in receipts if r.get('status')=='USABLE'),'refit':False,'retune':False},indent=2,sort_keys=True)+'\n')
