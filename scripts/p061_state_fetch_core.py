...
for sid in SHOTS:
    ...
    try:
        ...
        b=pd.DataFrame({'shot_id':sid,'split':split,'time':td});cov={}
...
summary={'shard':SHARD,'nshard':NSHARD,'requested_shots':len(SHOTS),'requested_ids':SHOTS,'successful_shots':sum(r['status']=='OK' for r in receipts),'failed_shots':sum(r['status']!='OK' for r in receipts),'rows':int(len(df)),'te_loaded_anywhere':False,'ne_loaded_anywhere':False,'unknown_is_zero':False,'excluded_secondary_provider':['pulse_schedule.i_plasma','pulse_schedule.n_e_line']}
